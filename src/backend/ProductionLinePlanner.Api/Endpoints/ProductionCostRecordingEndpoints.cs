...
public static class ProductionCostRecordingEndpoints
{
    ...
    public static void MapProductionCostRecordingEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/production/reports/daily", async (IProductionCostRecordingService service, DateOnly from, DateOnly to, Guid? orderId, Guid? modelId, Guid? workerId, CancellationToken ct) => Results.Ok(ApiResponse.Success(await service.DailyReportAsync(from, to, orderId, modelId, workerId, ct)))).RequireAuthorization().RequirePermission("production.view").RequireRateLimiting(ApiRateLimitPolicies.NormalRead).WithTags("Production reports");
    }
