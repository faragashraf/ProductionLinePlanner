...
export interface SubStageOption { id: string; mainStageId: string; productionLineId?: string; name: string; code: string; capacity: number; sequenceOrder: number; isActive: boolean; }
...
export class ManufacturingMasterDataApiService {
  ...
  searchSubStages(search = '', page = 1, pageSize = 50, isActive = true): Observable<SubStagePage> { return this.get<{ items: SubStageApiDto[]; totalCount: number; pageNumber: number; pageSize: number }>(`/api/stages?isActive=${isActive}&page=${page}&pageSize=${pageSize}&name=${encodeURIComponent(search)}`).pipe(map(result => ({ ...result, items: (result.items ?? []).map(item => this.toSubStageOption(item)) }))); }
  ...
  createOperationalStage(value: unknown): Observable<SubStageOption> { return this.postSub('/api/stages', value); } updateOperationalStage(id: string, value: unknown): Observable<SubStageOption> { return this.patchSub(`/api/stages/${id}`, value); } deactivateOperationalStage(id: string): Observable<unknown> { return this.post(`/api/stages/${id}/deactivate`, {}); } deleteOperationalStage(id: string): Observable<unknown> { return this.delete(`/api/stages/${id}`); } stageDependencies(id: string): Observable<StageDependencySummary> { return this.get(`/api/stages/${id}/dependencies`); }
  createDepartment(value: unknown): Observable<DepartmentItem> { return this.post('/api/departments', value); } updateDepartment(id: string, value: unknown): Observable<DepartmentItem> { return this.patch(`/api/departments/${id}`, value); } deleteDepartment(id: string): Observable<unknown> { return this.delete(`/api/departments/${id}`); }
