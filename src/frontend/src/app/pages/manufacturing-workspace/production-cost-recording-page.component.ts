...
export class ProductionCostRecordingPageComponent implements OnInit, OnDestroy {
  ...
  assignmentDraftUpdateMode: 'assignment-only' | 'draft-too' =
    'assignment-only';
  orderSearch = '';
  orderStatusFilter = '';
  editingOrderId = '';
  ...
  ngOnInit(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (error) =>
          this.handleError(error, 'تعذر تحميل موديلات الإنتاج.'),
      });
    ...
    this.recordForm.controls.productionDate.valueChanges
      ...
      .subscribe(() => {
        ...
        if (this.canViewAssignments && this.selectedSubStageId)
  ...
  get canRecord(): boolean {
    return this.permissionService.hasPermission(PERMISSIONS.production.record);
  }
  get canApprove(): boolean {
    return this.permissionService.hasPermission(PERMISSIONS.production.approve);
  }
  ...
  get canManageAssignments(): boolean {
    return this.permissionService.hasPermission(PERMISSIONS.assignments.manage);
  }
  get canViewAttendance(): boolean {
    return this.permissionService.hasPermission(PERMISSIONS.attendance.view);
  }
  get canSyncAttendance(): boolean {
  ...
  get isDraftContextUnavailable(): boolean {
    return !!this.draftContextUnavailable;
  }
  get hierarchyComplete(): boolean {
    return (
  ...
  get showWorkerPanel(): boolean {
    return !!this.selectedSubStageId;
  }
  get selectedProductionDate(): string {
    return this.recordForm.controls.productionDate.value || this.today;
  }
  get isSelectedProductionDateToday(): boolean {
    return this.selectedProductionDate === this.today;
  }
  get attendanceDate(): string {
    return this.selectedProductionDate;
  }
  get activeAttendanceWorkersCount(): number {
    return (
      this.workerContext?.activeServiceWorkersCount ??
      this.attendanceSnapshot?.items.length ??
      0
    );
  }
  get workersWithAttendanceCount(): number {
    return (
      this.workerContext?.workersWithAttendanceDataCount ??
      this.attendanceSnapshot?.items.filter(
        (worker) => worker.attendanceStatus !== 'Unassigned',
      ).length ??
      0
    );
  }
  get actualCheckInWorkersCount(): number {
    return (
      this.workerContext?.actualCheckInWorkersCount ??
      this.attendanceSnapshot?.items.filter(
        (worker) =>
  ...
  get noSourceCheckInWorkersCount(): number {
    return this.workerContext?.noSourceCheckInWorkersCount ?? 0;
  }
  get availableWorkersCount(): number {
    return this.workerContext?.availableWorkers.length ?? 0;
  }
  ...
  get assignmentMissingRequirements(): string[] {
    if (this.unassignDialogVisible) {
      return this.formSubmissionValidation.missingMessages(
        this.assignmentForm,
        this.assignmentRequiredFields(false, true),
      );
    }
    const worker = this.selectedAssignmentWorker;
    return this.formSubmissionValidation.missingMessages(
      this.assignmentForm,
      this.assignmentSubmissionRules(worker),
      [],
    );
  }
  get unavailableWorkersCount(): number {
    return this.workerContext?.unavailableWorkersCount ?? 0;
  }
  get hasAttendanceRecords(): boolean {
    return this.workersWithAttendanceCount > 0;
  }
  get hasActualCheckIns(): boolean {
    return this.actualCheckInWorkersCount > 0;
  }
  get showAttendanceEmptyState(): boolean {
    return (
  ...
  get lastSuccessfulAttendanceSyncLabel(): string {
    return this.lastSuccessfulAttendanceSyncAt
      ? new Intl.DateTimeFormat('ar-EG', {
          ...
          minute: '2-digit',
          second: '2-digit',
        }).format(new Date(this.lastSuccessfulAttendanceSyncAt))
      : '';
  }
  get assignmentCandidates(): AssignmentWorkflowWorker[] {
    ...
    return (this.workerContext?.presentWorkers ?? [])
      .filter(
        (worker) => worker.effectiveSubStageId !== this.selectedSubStageId,
      )
      .filter(
        (worker) =>
          !needle ||
  ...
  get currentWorkers(): AssignmentWorkflowWorker[] {
    return (this.workerContext?.currentWorkers ?? []).filter(
      (worker) => worker.assignmentType === 'Default',
    );
  }
  get recordingWorkers(): AssignmentWorkflowWorker[] {
    return this.currentWorkers.filter((worker) =>
      this.isRecordableWorker(worker),
    );
  }
  get selectedOrder(): ProductionOrder | undefined {
    return this.orders.find(
      (order) => order.id === this.recordForm.controls.productionOrderId.value,
    );
  }
  get selectedProductModelId(): string {
    return (
      this.selectedOrder?.productModelId ||
      this.orderForm.controls.productModelId.value ||
      ''
    );
  }
  ...
  get selectedProductLabel(): string {
    ...
    return model
      ...
      : this.selectedOrder?.productModelCode || 'غير محدد';
  }
  get selectedFactoryLabel(): string {
    const factory = this.factories.find(
      (item) => item.id === this.selectedFactoryId,
    );
    return factory ? `${factory.code} — ${factory.name}` : 'غير محدد';
  }
  get selectedProductionLineLabel(): string {
    const line = this.productionLines.find(
      (item) => item.id === this.selectedProductionLineId,
    );
    return line ? `${line.lineCode || '-'} — ${line.name}` : 'غير محدد';
  }
  get selectedMainStageLabel(): string {
    const stage = this.mainStages.find(
      (item) => item.id === this.selectedMainStageId,
    );
    return stage ? stage.name : 'غير محدد';
  }
  get selectedSubStageLabel(): string {
    const stage = this.subStages.find(
      (item) => item.id === this.selectedSubStageId,
    );
    return stage ? `${stage.code} — ${stage.name}` : 'غير محدد';
  }
  get selectedStageReadiness(): ProductStageReadiness | undefined {
    ...
    return this.productReadiness?.stages.find(
      (stage) =>
        stage.productModelStageId === stageId ||
        stage.subStageId === this.selectedSubStageId,
    );
  }
  get selectedAssignmentWorker(): AssignmentWorkflowWorker | undefined {
    return (this.workerContext?.presentWorkers ?? []).find(
      (worker) =>
        worker.workerId === this.assignmentForm.controls.workerId.value,
    );
  }
  get canPreviewIntake(): boolean {
    return (
      this.canRecord &&
      !!this.intakeStagesFile &&
      !!this.intakeSalaryFile &&
      !!this.intakeProductionFile &&
      this.intakeForm.valid &&
      !this.intakeLoading
    );
  }
  get hasOpenImportedDayIssues(): boolean {
    return (
      this.importedDayReview?.issues.some((issue) => issue.status === 'Open') ??
      false
    );
  }
  ...
  syncAttendanceToday(): void {
    ...
    (selectedDateSync
      ? selectedDateSync.call(this.attendance, productionDate)
      : this.attendance.syncToday()
    )
      .pipe(finalize(() => (this.attendanceSyncing = false)))
      .subscribe({
        next: (result) => {
        ...
        error: (error) => {
          ...
          if (this.isAttendanceSyncTimeout(error))
            this.refreshAfterAttendanceSyncAttempt(true);
        },
      });
  }

  selectIntakeFile(
    kind: 'stages' | 'salary' | 'production',
    event: Event,
  ): void {
  ...
  previewRealDataIntake(): void {
    ...
    this.intakeResult = null;
    this.api
      .previewRealDataIntake(this.realDataIntakeFormData())
      .pipe(finalize(() => (this.intakeLoading = false)))
      .subscribe({
        next: (preview) => (this.intakePreview = preview),
        error: (error) =>
          (this.intakeError =
            error.message ||
            'تعذر إنشاء معاينة الاستيراد. لم يتم حفظ أي بيانات.'),
      });
  }
  ...
  applyRealDataIntake(): void {
    ...
    if (
      !window.confirm(
        'سيتم تطبيق المعاينة الحالية كمسودات قابلة للمراجعة. هل تؤكد التطبيق؟',
      )
    )
      return;
    this.intakeLoading = true;
    this.intakeError = '';
    this.api
      .applyRealDataIntake(this.realDataIntakeFormData())
      .pipe(finalize(() => (this.intakeLoading = false)))
      .subscribe({
        next: (result) => {
        ...
        error: (error) =>
          (this.intakeError =
            error.message ||
            'تعذر تطبيق الاستيراد. تم التراجع عن أي تغيير غير مكتمل.'),
      });
  }
  ...
  openImportedDayReview(order: ProductionOrder): void {
    ...
    this.error = '';
    this.api
      .getProductionDayReview(order.id)
      .pipe(finalize(() => (this.importedDayReviewLoading = false)))
      .subscribe({
        next: (review) => {
          this.importedDayReview = review;
          this.importedDayResolutionReason = '';
          this.importedAllocationOverrideReasons = {};
        },
        error: (error) =>
          this.handleError(error, 'تعذر تحميل مراجعة يوم الإنتاج المستورد.'),
      });
  }

  markImportedStageNotOperated(productModelStageId: string): void {
    if (
      !this.importedDayReview ||
      !this.importedDayResolutionReason.trim() ||
      this.importedDayReviewLoading
    )
      return;
    this.importedDayReviewLoading = true;
    this.api
      .markStageNotOperated(
        this.importedDayReview.productionOrderId,
        productModelStageId,
        this.importedDayResolutionReason.trim(),
      )
      .pipe(finalize(() => (this.importedDayReviewLoading = false)))
      .subscribe({
        ...
        error: (error) =>
          this.handleError(error, 'تعذر حفظ قرار المرحلة غير المشغلة.'),
      });
  }

  participantOverrideKey(
    stageProductionRecordId: string,
    workerId: string,
  ): string {
    return `${stageProductionRecordId}:${workerId}`;
  }

  setImportedParticipantOverride(
    stageProductionRecordId: string,
    workerId: string,
  ): void {
    if (
    ...
    if (!reason) {
      this.error = 'سبب تفويض الحضور أو التسكين إلزامي.';
      return;
    }
    if (!window.confirm('سيتم حفظ سبب التفويض في لقطة اليوم المستورد. متابعة؟'))
      return;
    this.importedDayReviewLoading = true;
    this.api
      .setParticipantOverride(
        this.importedDayReview.productionOrderId,
        stageProductionRecordId,
        workerId,
        reason,
      )
      .pipe(finalize(() => (this.importedDayReviewLoading = false)))
  ...
  approveImportedDay(): void {
    ...
    this.importedDayReviewLoading = true;
    this.api
      .approveProductionDay(this.importedDayReview.productionOrderId)
      .pipe(finalize(() => (this.importedDayReviewLoading = false)))
      .subscribe({
        next: (review) => {
          this.importedDayReview = review;
        ...
        error: (error) =>
          this.handleError(
            error,
            'لا يمكن اعتماد اليوم قبل حل مشكلات المراجعة والحضور.',
          ),
      });
  }
        ...
        next: (lines) => {
          if (version === this.hierarchyRequestVersion)
            this.productionLines = lines.filter(
              (line) => line.isActive && line.factoryId === factoryId,
            );
        },
        error: (error) =>
          this.handleError(error, 'تعذر تحميل خطوط الإنتاج للمصنع المحدد.'),
      });
  }
  ...
  selectMainStage(mainStageId: string): void {
    ...
    this.masterData
      .subStagesForMainStage(mainStageId)
      .pipe(finalize(() => (this.loadingSubStages = false)))
      .subscribe({
        next: (stages) => {
          if (version === this.hierarchyRequestVersion)
            this.subStages = stages.filter(
              (stage) => stage.isActive && stage.mainStageId === mainStageId,
            );
        },
        error: (error) =>
          this.handleError(
            error,
            'تعذر تحميل المراحل الفرعية للمرحلة المحددة.',
          ),
      });
  }
  ...
  workflowOrders(): ProductionOrder[] {
    return this.orders.filter(
      (order) => order.productionLineId === this.selectedProductionLineId,
    );
  }
  ...
  loadOrderStages(keepStage = false): void {
    ...
    this.modelStages = [];
    if (!order || order.productionLineId !== this.selectedProductionLineId)
      return;
    this.refreshProductReadiness();
    ...
    this.api.listModelStages(order.productModelId).subscribe({
      next: (stages) => {
        if (version === this.stageRequestVersion) {
          this.modelStages = stages.filter(
            (stage) =>
              stage.isActive && stage.subStageId === this.selectedSubStageId,
          );
          this.invalidatePreviewForChangedDraft();
          if (
            keepStage &&
            !this.modelStages.some(
              (stage) =>
      ...
      error: (error) =>
        this.handleError(error, 'تعذر تحميل مرحلة الموديل للمسار المحدد.'),
    });
  }
  ...
  createOrder(): void {
    this.orderForm.controls.productionLineId.setValue(
      this.selectedProductionLineId,
    );
  ...
  activate(order: ProductionOrder): void {
    if (
      !this.canRecord ||
      order.productionLineId !== this.selectedProductionLineId
    )
      return;
    this.saving = true;
    this.api
  ...
  openRecord(record: StageProductionRecord): void {
    ...
    this.api.getRecord(record.id).subscribe({
      ...
      error: (error) => {
        this.saving = false;
        this.handleError(error, 'تعذر فتح سجل الإنتاج.');
      },
    });
  }
  ...
  addWorker(worker?: AssignmentWorkflowWorker): void {
    ...
    if (
      workerId &&
      !this.recordingWorkers.some((option) => option.workerId === workerId)
    ) {
      this.error =
        'لا يمكن إرفاق عامل إلا بعد أن يصبح ضمن التسكين الحالي للمرحلة.';
      return;
    }
  ...
  applyAssignment(): void {
    ...
    const result = this.createCurrentAssignment(worker, reason);
    this.assignmentSaving = true;
    this.assignmentSuccess = '';
    result.pipe(finalize(() => (this.assignmentSaving = false))).subscribe({
      next: () => {
        this.assignmentSuccess = 'تم حفظ التسكين الدائم للمرحلة المحددة.';
        this.assignmentForm.reset({
          workerId: '',
          reason: '',
        });
  ...
  selectAssignmentWorker(worker: AssignmentWorkflowWorker): void {
  ...
  confirmUnassign(): void {
    ...
    request.pipe(finalize(() => (this.assignmentSaving = false))).subscribe({
      next: () => {
        ...
        this.assignmentSuccess =
          'تم إلغاء التسكين الحالي وحُفظ السبب. لم تتغير أي دفعة إنتاج سابقة.';
        this.unassignDialogVisible = false;
        this.pendingUnassignWorker = null;
        this.clearDraftParticipantImpact();
        this.assignmentForm.reset({
      ...
      error: (error) =>
        this.handleError(
          error,
          'تعذر إلغاء تسكين العامل. حدّث البيانات وحاول مرة أخرى.',
        ),
    });
  }
  ...
  saveDraft(): void {
    if (
      this.saving ||
      this.restoringDraftContext ||
      this.isDraftContextUnavailable ||
      !this.canRecord ||
    ...
    if (!validation.valid) {
      this.error = this.isDraftContextUnavailable
        ? this.draftContextUnavailable
        : validation.summary;
      return;
    }
  ...
  calculatePreview(): void {
    ...
    this.api
      ...
      .subscribe({
        next: (preview) => {
          if (fingerprint === this.currentDraftFingerprint())
            this.setFreshPreview(preview);
  ...
  approve(record: StageProductionRecord): void {
    ...
    if (!this.hasConsistentEntitlementTotal(record)) {
      this.error =
        'لا يمكن اعتماد السجل لأن إجمالي المستحقات لا يطابق مجموع مستحقات العمال. أعد حساب المعاينة واحفظ المسودة من جديد.';
      return;
    }
  ...
  canApproveProductionRecord(record: StageProductionRecord): boolean {
    return (
      this.canApprove &&
      record.status === 'Draft' &&
      this.hasConsistentEntitlementTotal(record)
    );
  }

  openProductionApprovalCancellationDialog(
    record: StageProductionRecord,
  ): void {
  ...
  confirmProductionApprovalCancellation(): void {
    ...
    const reason =
      this.productionApprovalCancellationForm.controls.reason.value?.trim() ??
      '';
    const validation = this.formSubmissionValidation.validate(
      ...
      [
        {
          control: 'reason',
          message: 'سبب إلغاء اعتماد الإنتاج مطلوب',
          isMissing: () => !reason,
        },
      ],
    );
    if (
      ...
      this.error =
        validation.summary ||
        'لا يمكن إلغاء اعتماد هذا السجل في حالته الحالية.';
      return;
    }
    this.saving = true;
    this.recordActionSuccess = '';
    this.api
      .cancelProductionApproval(record.id, record.concurrencyToken, reason)
      .pipe(finalize(() => (this.saving = false)))
      .subscribe({
        next: (updated) => {
          this.applyRecord(updated);
          this.preview = updated;
          this.recordActionSuccess =
            'تم إلغاء اعتماد الإنتاج. بقي السجل ولقطاته المالية محفوظة للمراجعة.';
          this.productionApprovalCancellationDialogVisible = false;
          this.pendingProductionApprovalCancellation = null;
          this.productionApprovalCancellationForm.reset({ reason: '' });
          this.loadRecords();
  ...
  hasConsistentEntitlementTotal(
    record: Pick<StageProductionRecord, 'totalWorkerEarnings' | 'workers'>,
  ): boolean {
    const allocationsTotal = record.workers.reduce(
      (total, worker) => total + Number(worker.calculatedEarning || 0),
      0,
    );
    return (
      Math.abs(Number(record.totalWorkerEarnings || 0) - allocationsTotal) <
      0.00005
    );
  }
  ...
  hasValidQuantities(): boolean {
    const value = this.recordForm.getRawValue();
    return (
      Number(value.acceptedQuantity) + Number(value.rejectedQuantity) <=
      Number(value.producedQuantity)
    );
  }

  private assignmentRequiredFields(
    includeWorker = true,
    includeReason = false,
  ): RequiredFieldRule[] {
    const fields: RequiredFieldRule[] = [];
    if (includeWorker)
      fields.push({ control: 'workerId', message: 'العامل مطلوب' });
    if (includeReason)
      fields.push({
        control: 'reason',
        message: 'السبب مطلوب',
        isMissing: () => !this.assignmentForm.controls.reason.value?.trim(),
      });
    return fields;
  }

  private assignmentSubmissionRules(
    worker: AssignmentWorkflowWorker | undefined,
  ): RequiredFieldRule[] {
    const requiresReason = !!worker?.effectiveSubStageId && worker.effectiveSubStageId !== this.selectedSubStageId;
    return this.assignmentRequiredFields(true, requiresReason);
  }

  private recordSubmissionMessages(requireFreshPreview: boolean): string[] {
    const extra: string[] = [];
    if (!this.hasProductionContext())
      extra.push('اختر مسار الإنتاج وأمرًا نشطًا ومرحلة مطابقة');
    if (!this.hasValidQuantities())
      extra.push('يجب ألا تتجاوز الكمية المقبولة والمرفوضة الكمية المنتجة');
    if (this.workers.length === 0)
      extra.push('يجب إضافة عامل فعلي واحد على الأقل');
    if (!this.hasUniqueCurrentWorkersWithoutMessage())
      extra.push('حدّث قائمة العمال من التسكين الحالي للمرحلة');
    if (requireFreshPreview && !this.previewIsFresh)
      extra.push(
        this.previewStaleMessage ||
          (this.previewIsStale
            ? 'تم تغيير بيانات الدفعة. أعد حساب المعاينة.'
            : 'احسب المعاينة قبل حفظ المسودة'),
      );
    return this.formSubmissionValidation.missingMessages(
      this.recordForm,
  ...
  private hasUniqueCurrentWorkersWithoutMessage(): boolean {
    const selected = this.workers.controls
      .map((control) => control.get('workerId')?.value)
      .filter((workerId): workerId is string => !!workerId);
    return (
      new Set(selected).size === selected.length &&
      selected.every((workerId) =>
        this.recordingWorkers.some((worker) => worker.workerId === workerId),
      )
    );
  }
  ...
  private setFreshPreview(preview: StageProductionRecord): void {
    ...
    this.previewStaleMessage = this.previewIsFresh
      ? ''
      : 'نتيجة المعاينة غير متسقة: إجمالي المستحقات لا يطابق مجموع مستحقات العمال. أعد حساب المعاينة.';
  }
  ...
  private prepareDraftParticipantImpact(
    worker: AssignmentWorkflowWorker,
    action: 'remove' | 'replace',
  ): void {
    const isCurrentDraftParticipant =
      !this.isRecordReadOnly &&
      this.workers.controls.some(
        (control) => control.get('workerId')?.value === worker.workerId,
      );
  ...
  private applyDraftParticipantImpact(
    previousWorkerId: string,
    replacementWorkerId?: string,
  ): void {
    const indexes = this.workers.controls
      .map((control, index) =>
        control.get('workerId')?.value === previousWorkerId ? index : -1,
      )
    ...
    if (this.assignmentDraftUpdateMode === 'draft-too') {
      if (
        replacementWorkerId &&
        !this.workers.controls.some(
          (control) => control.get('workerId')?.value === replacementWorkerId,
        )
      ) {
        this.workers
          .at(indexes[0])
          .get('workerId')
          ?.setValue(replacementWorkerId);
        indexes
          .slice(1)
          .reverse()
          .forEach((index) => this.workers.removeAt(index));
      } else {
  ...
  trackByWorkerId(_: number, item: AssignmentWorkflowWorker): string {
    return item.workerId;
  }
  isRecordableWorker(worker: AssignmentWorkflowWorker): boolean {
    return (
      worker.attendanceStatus === 'Present' ||
      worker.attendanceStatus === 'Late'
    );
  ...
  workerAttendanceLabel(worker: AssignmentWorkflowWorker): string {
    if (worker.attendanceEvidence === 'ActualCheckInFound')
      return worker.attendanceStatus === 'Late'
        ? 'حاضر متأخر (بصمة مؤكدة)'
        : 'حاضر (بصمة مؤكدة)';
    if (worker.attendanceEvidence === 'ConfirmedAbsent') return 'غائب مؤكد';
    if (worker.attendanceEvidence === 'NoSourceCheckIn')
      return 'لا توجد بصمة مصدر / غير مؤكد';
  ...
  private loadWorkerContext(subStageId: string): void {
    ...
    this.assignments
      .getSubStageWorkerContext(subStageId, this.selectedProductionDate)
      .pipe(
        finalize(() => {
          if (
            version === this.workerContextRequestVersion &&
            this.selectedSubStageId === subStageId
          ) {
      ...
      .subscribe({
        ...
        error: () => {
          if (
            ...
            this.workerContextError =
              'تعذر تحميل العمال وحالة الحضور للمرحلة المحددة.';
          }
        },
      });
  }
  ...
  private loadAttendanceSummary(preserveExistingError = false): void {
    ...
    (selectedDateRead
      ? selectedDateRead.call(this.attendance, this.selectedProductionDate)
      : this.attendance.getToday()
    )
      .pipe(
        finalize(() => {
          if (
            version === this.attendanceRequestVersion &&
            this.selectedSubStageId === subStageId
          ) {
        ...
        error: () => {
          if (
            ...
            if (!preserveExistingError) {
              this.attendanceError =
                'تعذر تحميل حالة حضور اليوم. تحقق من الصلاحية أو حاول مرة أخرى.';
            }
          }
        },
      });
  ...
  private refreshAfterAttendanceSyncAttempt(
  ...
  readinessStatusLabel(status: string): string {
    ...
    if (status === 'CompensationNeedsReview')
  ...
  private loadProductLineReadiness(): void {
    ...
    this.api
      ...
      .pipe(
        finalize(() => {
          if (version === this.productReadinessRequestVersion)
      ...
      .subscribe({
        next: (readiness) => {
          if (version === this.productReadinessRequestVersion)
            this.productReadiness = readiness;
        },
        error: () => {
          if (version === this.productReadinessRequestVersion)
            this.productReadinessError =
              'تعذر تحميل ملخص الجاهزية الآن. أعد المحاولة؛ تبقى المسودة مفتوحة ولا تتغير بياناتها.';
        },
      });
  }
  ...
  private refreshWorkerContext(subStageId: string): void {
    ...
    this.assignments
      ...
      .subscribe({
        ...
        error: (error) => this.handleError(error, 'تعذر تحديث حالة عمال المرحلة.'),
      });
  }

  private createCurrentAssignment(
    worker: AssignmentWorkflowWorker,
    reason: string,
  ) {
  ...
  private routeContextFrom(
    params: ParamMap,
  ): ProductionRecordingRouteContext | null {
  ...
  private restoreRouteContext(context: ProductionRecordingRouteContext): void {
    ...
    this.masterData.factories().subscribe({
      next: (factories) => {
        ...
        this.factories = factories.filter((factory) => factory.isActive);
        const factory = this.factories.find(
          (item) => item.id === context.factoryId,
        );
        if (!factory) {
          this.markRouteContextUnavailable(
            'المصنع المحدد لم يعد متاحًا أو نشطًا. اختر مسار إنتاج جديدًا.',
          );
          return;
        }
        ...
        this.masterData
          ...
          .subscribe({
            next: (lines) => {
              ...
              this.productionLines = lines.filter(
                (line) => line.isActive && line.factoryId === factory.id,
              );
              const line = this.productionLines.find(
                (item) => item.id === context.productionLineId,
              );
              if (!line) {
                this.markRouteContextUnavailable(
                  'خط الإنتاج المحدد لا يتبع المصنع الحالي أو لم يعد نشطًا. اختر خطًا جديدًا.',
                );
                return;
              }
              ...
              this.masterData
                ...
                .subscribe({
                  next: (mainStages) => {
                    ...
                    if (!mainStage) {
                      this.markRouteContextUnavailable(
                        'المرحلة الرئيسية المحددة لا تتبع الخط الحالي أو لم تعد نشطة. اختر مرحلة جديدة.',
                      );
                      return;
                    }
                    ...
                    this.loadingSubStages = true;
                    this.masterData
                      .subStagesForMainStage(mainStage.id)
                      .pipe(
                        finalize(() => {
                          if (this.isCurrentRouteContextRestore(version))
                            this.loadingSubStages = false;
                        }),
                      )
                      .subscribe({
                        next: (subStages) => {
                          ...
                          this.subStages = subStages.filter(
                            (stage) =>
                              stage.isActive &&
                              stage.mainStageId === mainStage.id,
                          );
                          const subStage = this.subStages.find(
                            (item) => item.id === context.subStageId,
                          );
                          if (!subStage) {
                            this.markRouteContextUnavailable(
                              'المرحلة الفرعية المحددة لا تتبع المرحلة الحالية أو لم تعد نشطة. اختر مرحلة جديدة.',
                            );
                            return;
                          }
                        ...
                        error: () =>
                          this.handleRouteContextLoadError(
                            version,
                            'تعذر تحميل المراحل الفرعية للمسار المرسل. اختر المرحلة من جديد.',
                          ),
                      });
                  },
                  error: () =>
                    this.handleRouteContextLoadError(
                      version,
                      'تعذر تحميل المراحل الرئيسية للمسار المرسل. اختر المرحلة من جديد.',
                    ),
                });
            },
            error: () =>
              this.handleRouteContextLoadError(
                version,
                'تعذر تحميل خطوط الإنتاج للمسار المرسل. اختر الخط من جديد.',
              ),
          });
      },
      error: () =>
        this.handleRouteContextLoadError(
          version,
          'تعذر تحميل المصانع للمسار المرسل. اختر المصنع من جديد.',
        ),
    });
  }
  ...
  private handleRouteContextLoadError(version: number, message: string): void {
    if (this.isCurrentRouteContextRestore(version))
      this.markRouteContextUnavailable(message);
  }

  private restoreDraftContext(loaded: StageProductionRecord): void {
    const order = this.orders.find(
      (candidate) => candidate.id === loaded.productionOrderId,
    );
    ...
    if (!order) {
      ...
      this.draftContextUnavailable =
        'تعذر استعادة مسار هذه المسودة لأن أمر الإنتاج لم يعد متاحًا. لا يمكن حفظها قبل معالجة بياناتها المرجعية.';
    ...
    forkJoin({
      ...
      next: (context) => {
        ...
        const stage = context.modelStages.find(
          (candidate) => candidate.id === loaded.productModelStageId,
        );
        const subStage =
          stage &&
          context.subStages.find(
            (candidate) => candidate.id === stage.subStageId,
          );
        const mainStage =
          subStage &&
        ...
        const factory =
          ...
          context.factories.find(
            (candidate) => candidate.id === line.factoryId,
          );
        const validContext =
          !!stage &&
          !!subStage &&
        ...
        if (
          ...
          this.applyLoadedRecord(loaded);
          this.draftContextUnavailable =
            'تعذر استعادة مسار هذه المسودة لأن عنصرًا من المصنع أو الخط أو المراحل لم يعد نشطًا. تبقى بياناتها ظاهرة دون فقدان.';
          return;
        }

        this.selectedFactoryId = factory.id;
        this.productionLines = context.lines.filter(
          (candidate) =>
            candidate.factoryId === factory.id && candidate.isActive,
        );
        this.selectedProductionLineId = line.id;
        ...
        this.subStages = context.subStages.filter(
          (candidate) =>
            candidate.mainStageId === mainStage.id && candidate.isActive,
        );
        this.selectedSubStageId = subStage.id;
        this.modelStages = context.modelStages.filter(
          (candidate) =>
            candidate.subStageId === subStage.id && candidate.isActive,
        );
        this.applyLoadedRecord(loaded);
        this.loadAttendanceSummary();
        this.loadWorkerContext(subStage.id);
      ...
      error: (error) => {
        ...
        this.applyLoadedRecord(loaded);
        this.draftContextUnavailable =
          'تعذر تحميل المسار المرجعي للمسودة. تحقق من اتصال البيانات قبل الحفظ.';
        this.restoringDraftContext = false;
        this.saving = false;
  ...
  private applyLoadedRecord(loaded: StageProductionRecord): void {
    ...
    loaded.workers.forEach((worker) =>
      this.workers.push(
        this.workerGroup(
          worker.workerId,
          worker.percentage ?? null,
          worker.fixedAmount ?? null,
          worker.notes ?? '',
        ),
      ),
    );
  ...
  private ensureProductionContext(): boolean {
    ...
    if (!valid)
      this.error =
        'اختر المصنع والخط والمراحل وأمرًا نشطًا ومرحلة موديل مطابقة قبل تسجيل الإنتاج.';
    return valid;
  }
  ...
  private hasUniqueCurrentWorkers(): boolean {
    const selected = this.workers.controls
      .map((control) => control.get('workerId')?.value)
      .filter((workerId): workerId is string => !!workerId);
    const allCurrent = selected.every((workerId) =>
      this.recordingWorkers.some((worker) => worker.workerId === workerId),
    );
    if (!allCurrent)
      this.error = 'تم تغيير التسكين الحالي. حدّث قائمة العمال قبل الحفظ.';
  ...
  private confirmContextReset(): boolean {
    const hasUnsavedRecord =
      !this.isRecordReadOnly &&
      (this.recordForm.dirty ||
        this.workers.length > 0 ||
        !!this.editingRecordId);
    const hasUnsavedAssignment = this.assignmentForm.dirty;
    const hasUnsavedOrder = this.orderForm.dirty;
    return (
      !(hasUnsavedRecord || hasUnsavedAssignment || hasUnsavedOrder) ||
      window.confirm(
        'سيؤدي تغيير المسار إلى مسح مسودة التسكين أو الإنتاج أو الأمر غير المحفوظة. هل تريد المتابعة؟',
      )
    );
  }
  ...
  private confirmAssignmentActionReset(): boolean {
    return (
      !this.assignmentForm.dirty ||
      window.confirm(
        'سيتم استبدال مسودة التسكين غير المحفوظة بإجراء العامل المحدد. هل تريد المتابعة؟',
      )
    );
  }
  ...
  private workerGroup(
    ...
    return this.fb.group({
      workerId: [workerId, Validators.required],
  ...
  private attendanceSyncFailureMessage(error: unknown): string {
    ...
    const transportType = (
      response.error as { type?: string } | undefined
    )?.type?.toLowerCase();

    if (code === 'AttendanceSyncInProgress') return 'المزامنة جارية بالفعل.';
    if (
      code === 'AttendanceSyncTimeout' ||
      code === 'AttendanceSourceTimeout' ||
      response.status === 504 ||
      this.isAttendanceSyncTimeout(error)
    ) {
    ...
    if (response.status === 401)
      return 'انتهت جلسة المستخدم. سجّل الدخول ثم حاول مرة أخرى.';
    if (transportType === 'abort' || normalized.includes('abort'))
      return 'تم إلغاء طلب مزامنة الحضور من المتصفح. تحقق من الاتصال ثم أعد المحاولة.';
    if (
      code === 'AttendanceSourceError' ||
      code === 'AttendanceSyncCancelled' ||
      response.status === 0 ||
      normalized.includes('attendance source')
    ) {
      return 'تعذر الاتصال بمصدر البصمة. تحقق من اتصال مصدر الحضور ثم حاول مرة أخرى.';
    }
    if (
      normalized.includes('no attendance') ||
      normalized.includes('no check-in')
    ) {
      return 'لم يتم العثور على سجلات حضور لهذا اليوم في مصدر البصمة.';
    }

    if ((response.status ?? 0) >= 500)
      return 'حدث خطأ بالخادم أثناء مزامنة الحضور. حاول مرة أخرى.';
  ...
  private handleError(
    error: { status?: number; message?: string; error?: unknown },
    fallback: string,
  ): void {
    this.error =
      error.status === 409
